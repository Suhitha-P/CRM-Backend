...
router.post("/login", async (req, res) => {
    ...
    if (user && !user.isVerified) {
        return res.json({
          status: "error",
          message:
            "Your account has not been verified. Please check your email and verify your account before being able to log in!",
        });
    } 
      
	const passFromDb = user && user._id ? user.password : null;

    if (!passFromDb || !bcrypt.compareSync(password, passFromDb)) {
